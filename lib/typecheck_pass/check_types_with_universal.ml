...
and check_abstraction expected_type param_decls body_expr =
  ...
  let expect_equal_param_types (expected, actual) =
    if are_types_compatible expected actual then return ()
...
and check_type_application expected_type expr given_types =
  let* expr_t = check_expr None expr in
  match expr_t with
  | TypeForAll (tvs, t) -> (
      match List.zip tvs given_types with
      | Ok tv_target_pairs ->
          let result_t = type_substitute t tv_target_pairs in
      ...
      | Unequal_lengths ->
          let err =
            Error_incorrect_number_of_type_arguments
              {
                type' = expr_t;
                expected = List.length tvs;
                actual = List.length given_types;
              }
          in
          error err)
  | _ -> error (Error_not_a_generic_function expr_t)

and check_type_abstraction expected_type tvs expr =
  let* expr_t = check_expr None expr in
...
and check_decl decl : unit t =
  ...
  @@
  match decl with
  | DeclFunGeneric (_, _, _, params, ret_type, _, body_decls, return_expr)
...
let make_globals_type_map (AProgram (_, _, decls)) =
  let pairs =
    List.filter_map decls ~f:(function
      ...
      | DeclFunGeneric (_, ident, tvs, params, ret_typ, _, _, _) ->
          let fun_t = extract_fun_type params ret_typ in
          let t = TypeForAll (tvs, fun_t) in
          Some (ident, t)
      | _ -> None)
  in
